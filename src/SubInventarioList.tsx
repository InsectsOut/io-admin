...
const SubInventarioList: React.FC<SubInventarioListProps> = ({ title, icon, subinventarios, onSelect, onAdd, flag, organizacion }) => {
    ...
    return (
        <SectionContainer>
            ...
            <EntryList>
                {inventarios.map((inv) => (
                    <EntryItem key={inv.id}
                        onClick={() =>{ onSelect(inv.id); pushToQueryParams(empleados.find(emp => emp.id === inv.tecnico_id)?.nombre || "Técnico Desconocido", inv.id) }}
                    >
                        {flag === InventarioFlag.tecnicos &&
                            <EntryText>
                                {empleados.find(emp => emp.id === inv.tecnico_id)?.nombre}
                            </EntryText>
                        }
                        {flag === InventarioFlag.principal &&
                            <EntryText>
                                {inv.inv_nombre || "Inventario Principal"}
                            </EntryText>
                        }
                        {flag === InventarioFlag.vehiculos &&
                            <EntryText>
                                {inv.inv_nombre || "Inventario vehícular"}
                            </EntryText>
                        }
                        {flag === InventarioFlag.equipo &&
                            <EntryText>
                                {inv.inv_nombre || "Inventario de equipo"}
                            </EntryText>
                        }
                    ...
                    <ModalContent>
                        ...
                        <ModalForm >
                            ...
                            <CardInputs
                                ...
                                autoFocus={true}
                            ></CardInputs>
                            <ModalButton onClick={() => createInventario()} type="button">Crear</ModalButton>
                        </ModalForm>
