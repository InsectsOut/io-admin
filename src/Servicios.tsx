...
export const ServiciosElement5 = styled.div<{ screen_width?: number; swipeActiator?: boolean }> /*style*/ `
    ...
    border-right: ${props =>
        props.swipeActiator ? "4px solid rgba(0, 0, 0, 0.1)" : "none"}; /* Light border when swiped */
    box-shadow: ${props =>
        props.swipeActiator ? "-4px 0 8px rgba(0, 0, 0, 0.2)" : "none"}; /* Shadow to simulate behind effect */
    #borrarServicio {
        all: unset;
        display: ${props => (props.screen_width && props.screen_width >= 900 ? "block" : "none")};
        width: 2rem;
...
export const Servicios: React.FC<serviciosProps> = props => {
    ...
    const fetchConteo = async () => {
        const { count } = await supabase.from("Servicios").select("id", { count: "exact" });
        setPaginasNoFilter(() => count);
    ...
    return (
        <>
            ...
            <ServiciosContainer>
                <Titulo tabIndex={-1}>Servicios</Titulo>
                <SearchBarForm>
                    <SearchBar
                        type="text"
                        name="barra"
                ...
                <FiltrosContainer>
                    <FiltrosLeft>
                        ...
                        >
                            Fecha <FlechaAbajo className={isRotated3 ? "rotated3" : ""} />
                        </FiltrosLista>
                        <FiltrosLista
                            id="estatusFilter"
                            onClick={(event: React.MouseEvent<HTMLLIElement, MouseEvent>) => {
                        ...
                        >
                            Estatus <FlechaAbajo className={isRotated4 ? "rotated4" : ""} />
                        </FiltrosLista>
                        <FiltrosLista
                            // id="estatusFilter"
                            onClick={(event: React.MouseEvent<HTMLLIElement, MouseEvent>) => {
                        ...
                        >
                            Técnico <FlechaAbajo className={isRotated5 ? "rotated5" : ""} />
                        </FiltrosLista>
                        {modalVisible && (
                            <ModalContainer
                                open={modalVisible}
                            ...
                            >
                                        ...
                                        <ModalContentBottom open={modalVisible}>
                                            <div className="filtroActionButtons">
                                                ...
                                                <button
                                                    ...
                                                    id="aplicar"
                                                    onClick={() => {
                                                        handleSetText(["cliente"], [clientId?.toString() ?? ""]).then(
                                                            () => {
                                                                // filterServicios()
                                                                handlePageSetter();
                                                                setIsRotated(false);
                                ...
                                {textModal === "fecha" && (
                                    <>
                                        <ModalContentTop open={modalVisible}>
                                            <div>
                                                <p style={{ color: "#727272", marginBottom: "0" }}>
                                                    ...
                                                    {empleados
                                                        ...
                                                        .map(empleado => (
                                                            <ClientName
                                                                key={empleado?.id}
                                                                onClick={() => {
                                                                    handleTecnicoClick(empleado?.id);
                                                                }}
