...
const Calendar: React.FC<calendarProps> = props => {
    ...
    const fetchEmpleados = async () => {
        try {
            let query = await supabase.from("Empleados").select(`*`).filter("organizacion", "eq", props.organizacion);
            const { data, error } = query;
            if (error) {
                console.log(error);
            }
    ...
    const fetchClientes = async () => {
        try {
            let query = await supabase.from("Clientes").select(`*`).filter("organizacion", "eq", props.organizacion);
            const { data, error } = query;
            if (error) {
                console.log(error);
            }
    ...
    const applyFilter = () => {
        ...
        if (selectedAplicador !== undefined && selectedAplicador !== 0) {
            filtered = filtered.filter(servicio => servicio.Empleados?.id === selectedAplicador);
        }
                    ...
                    {modalActive && (
                        <Modal ref={modalRef}>
                            ...
                            <div className="filtroActionButtons">
                                <button className="actionButtonsStyles" id="limpiar" onClick={clearFilter}>
                                    Limpiar
                                </button>
                                <button
